...
/* Pi Mod by Desno365 */
// P.S.: this mod was made during the Pi Day: 14 March 2015: 3:14:15


// decimal precision of the Pi
var selectedPrecision = 1000; // 1 to 1e+9 inclusive
...
function newLevel()
{
	if(!threadStarted)
	{
		...
		{ 
			run: function()
			{
				Decimal.config({precision: selectedPrecision});
				...
				for(var k = new Decimal(0); k.lte(precision); k = k.plus(one))
				{
					...
					updateDisplayedPi(piString.substring(0, currentDecimal));
					currentDecimal++;
				}	
			}
		}).start();
...
function modTick()
{
	if(senseiSpawned)
	{
		Entity.setVelX(sensei, 0);
		Entity.setVelY(sensei, 0);
		Entity.setVelZ(sensei, 0);
	}
}

var textView;
function displayPi(string)
{
	currentActivity.runOnUiThread(new java.lang.Runnable()
	{
		run: function()
		{
			try
			{
				...
				popup.setTitle(new android.text.Html.fromHtml("The Pi"));
				popup.setCanceledOnTouchOutside(false);

				textView = new android.widget.TextView(currentActivity);
				textView.setText(string);
				layout.addView(textView);

				var spawnPi = new android.widget.Button(currentActivity); 
				spawnPi.setText("Spawn the Pi Sensei");
				spawnPi.setOnClickListener(new android.view.View.OnClickListener()
				{
					onClick: function()
					{
                        if(senseiSpawned)
                            Entity.remove(sensei);
						sensei = Level.spawnMob(Player.getX() + (Math.random() - 0.5), Player.getY() - 1, Player.getZ() + (Math.random() - 0.5), 15);
						senseiSpawned = true;
						Entity.setHealth(sensei, 99999); // he doesn't die!
						Entity.setNameTag(sensei, string);
						popup.dismiss();
					}
				});
				layout.addView(spawnPi);
...
function updateDisplayedPi(string)
{
	currentActivity.runOnUiThread(new java.lang.Runnable()
	{
		run: function()
